        ...
        <div className="container">
          ...
          <div className="practice-grid">{practices.map((practice, index) => {
            return (
              ...
              >
                <div className="practice-photo"><img src={practice.image} alt={`${practice.name} class at Transform Active, Mullumbimby`} style={{ objectPosition: practice.imagePosition }} loading="lazy" decoding="async" /></div>
  ...
  return (
    <section id="teachers" className="section instructors-section">
      <div className="container">
        ...
        <div className="teacher-grid">{teachers.map((teacher, index) => <Reveal key={teacher.name} className="teacher-card" delay={index * 0.1}><div className="teacher-photo"><img src={teacher.image} alt={`${teacher.name}, ${teacher.role} at Transform Active`} style={{ objectPosition: teacher.imagePosition }} width="240" height="240" loading="lazy" decoding="async" /><span className="teacher-photo-mark"><Sprout size={20} strokeWidth={1.25} aria-hidden="true" /></span></div><h3>{teacher.name}</h3><p className="teacher-specialty">{teacher.role}</p><p className="teacher-bio">{teacher.bio}</p><div className="certifications">{teacher.certifications.map((certification) => <span key={certification}>{certification}</span>)}</div></Reveal>)}</div>
...
function Footer({ onBook, onInfo }: { onBook: (intent: BookingIntent) => void; onInfo: (page: InfoPage) => void }) {
  return (
    <footer className="site-footer"><div className="container"><div className="footer-grid">
      ...
      <div><h3>The studio</h3><ul><li><button type="button" onClick={() => onInfo('About the studio')}>About us</button></li><li><a href="#teachers">Our teachers</a></li><li><a href="#schedule">Timetable</a></li><li><a href="#sauna">Infrared sauna</a></li><li><a href="#toolkit">Free toolkit</a></li><li><a href="#membership">Membership</a></li><li><button type="button" onClick={() => onInfo('Class FAQs')}>Class FAQs</button></li></ul></div>
